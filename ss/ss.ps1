...
function ss {
 ...
 Examples:
    ...
    if ((-NOT (test-path $DBfile)) -OR (((Get-Date) - (gci $DBfile).LastWriteTime).Minutes -ge 30)) {
        aria2c --allow-overwrite=true https://github.com/Virakal/ScoopMaster/releases/download/Databases/AllAppsDB.7z -d "$env:TEMP" | Out-Null
    }
    ...
    if ($oLast) {
        $csv = if ($oldPS) { $csv | Select-String "Virakal/ScoopMaster" } else { $csv | Select-String "Virakal/ScoopMaster" -raw }
    }
    if ($oOfficial) {
    ...
    Foreach ($line in $table) {
        ...
        $line.Bucket = $line.Bucket -Replace "(^.*/Virakal/ScoopMaster)", "$cSMaster`$1$cNormal"
